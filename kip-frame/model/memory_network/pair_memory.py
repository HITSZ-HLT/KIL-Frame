...
class Pair_Memory(base_model):
    def __init__(self,mem_slots,input_size, output_size, key_size, head_size, num_heads=4):
        ...
        self.rel_projector = nn.Linear(input_size, input_size)
    ...
    def forward(self, input, proto_rel, proto_des):
        rep = self.attention_over_memory(input, proto_des)
        # [B, 768]
        rep = rep[:, -1, :]
        # [B, rels_num]
        f_proto_rel = self.rel_projector(proto_rel)
        mem_score = torch.matmul(rep, f_proto_rel.t())
        return mem_score
